...
FitLasso <- function(X, Y, intercept = TRUE) {
  ...
  mod <- try(glmnet(X, Y, standardize = FALSE, intercept = intercept,
                    ...
                    upper.limits = 1,  # any given word can't be more than 1
  ...
  # cat(stderr(), "\nhere is Y: ", as.character(Y), "\n")


  # cat(stderr(), "\nhere are the coefs from regression: ", as.character(coef(mod)), "\n")
  # cat(stderr(), "\nhere is summary of regression: ", as.character(print(mod)), "\n")
...
FitDistribution <- function(estimates_stds, map, quiet = FALSE) {
  ...
  coefs
  cat("\nThen the linear model found these coefficient values:\n")
  print(coefs)
}
